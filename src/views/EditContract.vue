<template>
    <div>
        <div class="group">
            ...
            <div class="grid grid-cols-2 gap-x-4">
                <div class="subgroup">
                    ...
                    <div class="mb-2">
                        ...
                        <Dropdown v-model="customerObj" editable filter :options="customerList.data" optionLabel="customer_name" placeholder="" id="dropdown"
                    ...
                    <div class="">
                        ...
                        <Dropdown v-model="salesObj" filter :options="salesList.data" optionLabel="username" placeholder="" id="dropdown"
                        pt:input:class="p-0 text-sm" pt:clearIcon:class="text-black" pt:trigger:class="text-black"
                        class="inputBox"/> 
                    </div>
                    ...
                    <div class="">
                        ...
                        <input type="text" class="inputBox" v-model="projectDetail.cost_sheets">
                    </div>

                    <div class="">
                        <label for="" class="block">Project Name<span class="text-red-600">*</span></label>
                        <input type="text" class="inputBox" :autofocus="isDuplicate" v-model="projectDetail.project_name">
                    </div>
                </div>
                <div>
                    <div class="subgroup">
                        ...
                        <div class="grid grid-cols-2 gap-x-6 ">
                            <div class="">
                                ...
                                <Dropdown v-model="projectDetail.project_type" :options="contractTypeList" placeholder="" id="dropdown"
                                pt:input:class="p-0 text-sm" pt:clearIcon:class="text-black" pt:trigger:class="text-black"
                                class="inputBox"/> 
                            ...
                            <div class="">
                                ...
                                <input type="text" class="inputBox" v-model="projectDetail.contract_number">
                            </div>
                            <div>
                                <label for="onSiteEngineer">Managed Service<span class="text-red-600"></span></label> <br/>
                                <input id="onSiteEngineer" type="checkbox" v-model="projectDetail.on_site_engineer">
                            </div>
                    ...
                    <div class="subgroup">
                        ...
                        <div class="">
                            <div class="">
                                <label for="" class="block">Amount Reserve for Internal Cost<span
                                        class="text-red-600">*</span></label>
                                <div class="flex">
                                    <span class="bg-gray-400 text-gray-200 rounded-sm p-2 my-2">Rp.</span>
                                    <InputNumber class="w-full p-0 self-center focus:outline-none rounded-none text-sm" v-model="projectDetail.internal_cost" inputId="integeronly" />
                                </div>
                            </div>
                            ...
                            <div class="">
                                ...
                                <div class="flex">
                                    <span class="bg-gray-400 text-gray-200 rounded-sm p-2 my-2">Rp.</span>
                                    <InputNumber class="w-full p-0 self-center focus:outline-none rounded-none text-sm" v-model="projectDetail.selling_prices" inputId="integeronly" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="subgroup">
                <div class="flex justify-between pb-4">
                    <h2>Product Info</h2>
                    <button class=" bg-[#908c13] text-white py-2 px-4 rounded-xl" v-on:click="showProductDetail('')">
                ...
                <table class="min-w-full" v-if="productInfo.length != 0">
                    ...
                    <tbody class="min-w-full overflow-y-auto block max-h-96">
                        <tr v-for="(product, index) in productInfo" v-on:click="showProductDetail(product.product_id)"
                            ...
                            <td><button class="pi pi-trash" v-on:click="deleteProduct(product.product_id)"></button></td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="grid grid-cols-2 gap-x-4">
                    <div class="">
                        ...
                        <textarea name="" id=""
                            ...
                            v-model="projectDetail.description"></textarea>
                    </div>
                    <div class="">
        ...
        <div class="w-full flex justify-end gap-2">
            <button v-if="projectDetail.project_status == 'Draft'" class="bg-[#908c13] p-2 w-32 text-white rounded-md" @click="saveContract(status='Draft')">Save Draft</button>
            <button class="bg-green-500 p-2 w-24 text-white rounded-md" @click="saveContract(status='Pending')">Submit</button>
        </div>
    </div>
    ...
    <div>
        <AddProduct v-model:showAddProduct="showAddProduct" v-model:productDetail="productDetail" v-if="showAddProduct"
            v-model:successAdd="successAdd" v-model:reopen="reopen"/>
    </div>
    <div>
        <ProductDetail v-model:productDetail="productDetail" v-model:showDetail="showDetail" />
    </div>
    <Toast/>
...
const CM = defineAsyncComponent(() => import('../components/CorrectiveMaintenance.vue'))
const PM = defineAsyncComponent(() => import('../components/PreventiveMaintenance.vue'))
...
const deleteProduct = async (product_id) => {
    try {
        const response = await axios.delete(import.meta.env.VITE_API_URL+`product/${product_id}`, {
            headers: {
                "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
            }
        })
        toast.add({severity: 'success', summary: 'Success Message', detail: "Success delete product", life: 5000})
        showAddProduct.value = !showAddProduct.value
        location.reload()
    } catch (error) {
        toast.add({severity: 'Error', summary: 'Failed Message', detail: "Failed delete product", life: 5000})
    } 
}

const addCustomer = async () => {
    if(typeof customerObj.value=="string" && customerObj.value!=""){
        var checkExist = customerList.value.data.find(({customer_name}) => customer_name.toLowerCase() === customerObj.value.toLowerCase())
        if(checkExist!=undefined){
            ...
            if (confirm(text) == true) {
                ...
                customerObj.value = customerList.value.data.find(({customer_id}) => customer_id === projectDetail.value.customer_id)
            }
        }
    }
}
...
watch(successAdd, async () => {
    const result = [{}]
    var response = await axios.get(import.meta.env.VITE_API_URL + `project/${route.params.id}`)
        .then(data => {
...
const getSales = async() => {
    try{
        ...
        salesObj.value = salesList.value.data.find(({id}) => id === projectDetail.value.sales_person_id)
    }catch (error) {
        console.error('Error fetching data:', error);
    }
}
...
const showToast = async () => {
    ...
    if(isCopy === "copy") {
        toast.add({severity:"info", summary:"Please make sure to update your: \n- Project Name\n- Contract Number\n- Cost Sheet", detail:"Note: Information will disappear after complete the following information"})
...
const validate = () => {
    ...
    if(projectDetail.value.customer_name!="" &&
    ...
    projectDetail.value.contract_number!="" &&
        ...
        if((cm.value.cm_by=='None' || (cm.value.cm_by!='None' && (cm.value.start_date != '' && cm.value.end_date != '' && (cm.value.quantity.includes('Ticket') || cm.value.quantity.includes('Man Days'))))) &&
            (pm.value.pm_by=='None' || (pm.value.pm_by!='None' && (pm.value.start_date != '' && pm.value.end_date != '' && pm.value.quantity != "" && (pm.value.quantity.includes('Ticket') || pm.value.quantity.includes('Man Days')))))&&
...
const saveContract = async (status) => {
    ...
    try {
        ...
        console.log("asd")
        const projectResponse = await axios.patch(import.meta.env.VITE_API_URL + `project/${route.params.id}`, projectDetail.value, {
            headers: {
                "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
            }
        })
        console.log("asd")
        const maintenanceResponse = await axios.patch(import.meta.env.VITE_API_URL+'parent-maintenance', maintenance, {
            headers:{
                "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token,
                'project-id': route.params.id
            }
        ...
        for(let i in finalFileList.value){
            if(finalFileList.value[i].id == undefined){
                ...
                try{
                    ...
                    toast.add({severity: 'error', summary: 'Error Message', detail: e.response.data.message, life: 5000})
                    loading.value = false
                    return
                }
            }  
