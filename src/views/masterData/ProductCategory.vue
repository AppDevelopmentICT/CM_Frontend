<template>
    <div class="flex flex-row justify-between px-8">
        <h1>Product Cateory</h1>
        <button v-on:click="addCategory" class="bg-[#908c13] p-2 text-white flex items-center rounded-md">
            ...
            <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <tr v-for="category in currList" :key="category.category_id"
                                ...
                                <button class="bg-red-600 px-2 m-2 rounded-md text-white"
                                    v-on:click="deleteCategory(category.category)">Delete</button>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="fixed inset-0 flex items-center  justify-center bg-gray-800 bg-opacity-75" v-if="showModal">
        <div class="bg-white rounded-lg p-8 max-w-md">
            <div class="flex justify-end">
                <button class="pi pi-times " v-on:click="addCategory"></button>
            </div>
            <h1 class=" text-center mt-4 text-[#908c13] font-semibold text-2xl mb-4">Add category</h1>
            <div class="py-2">
                <label class="block">Category Name<span class="text-red-600">*</span></label>
            ...
            <div class="flex justify-end">
                <button class="bg-[#908c13] text-white px-2 py-1 rounded-md "
                    v-on:click="submitCategory">submit</button>
            </div>
        </div>
...
watch(searchInput, (x) => {
    ...
    if(searchInput.value == ''){
        ...
        searchList = categoryList.value.filter((list) => list.category_name.toLowerCase().includes(searchInput.value.toLowerCase()))
    }
...
const deleteCategory = async (category_name) => {
    ...
    if (confirm(text) == true) {
        try {
            const response = await axios.delete(import.meta.env.VITE_API_URL + `category/${category_name}`,{
                headers: {
...
const submitCategory = async () => {
    ...
    const testCategory = { category: categoryName.value }
    try {
        const response = await axios.post(import.meta.env.VITE_API_URL + "category", testCategory, {
            headers: {
                "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
            }
        })
