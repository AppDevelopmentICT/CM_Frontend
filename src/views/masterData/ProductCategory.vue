            ...
            <tbody>
              ...
              >
                ...
                <button
                  ...
                  v-on:click="deleteCategory(category.category)"
                >
                  Delete
                </button>
              </tr>
  ...
  >
    <div class="bg-white rounded-lg p-8 max-w-md">
      ...
      <div class="py-2">
        ...
        >
        <input type="text" class="inputBox" v-model="categoryName" />
      </div>

      <div class="flex justify-end">
        <button
          class="bg-[#908c13] text-white px-2 py-1 rounded-md"
          v-on:click="submitCategory"
        >
          submit
        </button>
      </div>
...
watch(searchInput, (x) => {
  if (searchInput.value == "") {
    ...
    searchList = categoryList.value.filter((list) =>
      list.category_name
        .toLowerCase()
  ...
  if (confirm(text) == true) {
    try {
      const response = await axios.delete(
        import.meta.env.VITE_API_URL + `category/${category_name}`,
        {
          headers: {
