<template>
    <div class="flex flex-row justify-between px-8">
        <h1>PM</h1>
        <button v-on:click="addPM" class="bg-[#908c13] p-2 text-white flex items-center rounded-md">
            ...
            <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <tr v-for="PM in currList" :key="PM.name"
                                class="flex justify-between px-4">
                                <td class="text-sm text-gray-900 font-light whitespace-nowrap">{{ PM.name }}</td>
                                <button class="bg-red-600 m-2 px-2 rounded-md text-white"
                                    v-on:click="deletePM(PM.name)">Delete</button>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="fixed inset-0 flex items-center  justify-center bg-gray-800 bg-opacity-75" v-if="showModal">
        <div class="bg-white rounded-lg p-8 max-w-md">
            ...
            <h1 class=" text-center mt-4 text-[#908c13] font-semibold text-2xl mb-4">Add PM</h1>
            <div class="py-2">
            ...
            <div class="flex justify-end">
                <button class="bg-[#908c13] text-white px-2 py-1 rounded-md " v-on:click="submitPM">submit</button>
            </div>
        </div>
...
watch(searchInput, (x) => {
    ...
    if(searchInput.value == ''){
        ...
        searchList = pmList.value.filter((list) => list.name.toLowerCase().includes(searchInput.value.toLowerCase()))
    }
...
const deletePM = async (PM_id) => {
    ...
    if (confirm(text) == true) {
        try {
            const PMId = { id: PM_id }
            const response = await axios.delete(import.meta.env.VITE_API_URL + `pm_by/${PM_id}`,{
                headers: {
...
const submitPM = async () => {
    ...
    try {
        const response = await axios.post(import.meta.env.VITE_API_URL + "pm_by", testPM,{
            headers: {
                "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
            }
        })
