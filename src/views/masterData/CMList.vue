  ...
  >
    <div class="bg-white rounded-lg p-8 max-w-md">
      ...
      <div class="py-2">
        ...
        <input type="text" class="inputBox" v-model="CMName" />
      </div>
...
watch(searchInput, (x) => {
  if (searchInput.value == "") {
    ...
    searchList = cmList.value.filter((list) =>
  ...
  if (confirm(text) == true) {
    try {
      ...
      const response = await axios.delete(
        import.meta.env.VITE_API_URL + `cm_by/${CM_id}`,
        {
          headers: {
...
const submitCM = async () => {
  ...
  try {
    const response = await axios.post(
      import.meta.env.VITE_API_URL + "cm_by",
      testCM,
      {
        headers: {
          "user-token": JSON.parse(localStorage.getItem("new_token"))
