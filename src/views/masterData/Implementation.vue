      ...
      <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
        <div class="overflow-hidden">
          <table class="min-w-full">
            ...
            <tbody>
              <tr
                v-for="Implementation in currList"
                :key="Implementation.implementation_type"
              ...
              >
                ...
                <button
                  ...
                  v-on:click="
                    deleteImplementation(Implementation.implementation_type)
                  "
                >
                  Delete
                </button>
  ...
  >
    <div class="bg-white rounded-lg p-8 max-w-md">
      ...
      <h1 class="text-center mt-4 text-[#908c13] font-semibold text-2xl mb-4">
        Add Implementation By
      </h1>
      <div class="py-2">
        <label class="block"
          >Implementation By<span class="text-red-600">*</span></label
        >
        <input type="text" class="inputBox" v-model="ImplementationBy" />
      </div>

      <div class="flex justify-end">
        <button
          class="bg-[#908c13] text-white px-2 py-1 rounded-md"
          v-on:click="submitImplementation"
        >
          submit
        </button>
      </div>
...
watch(searchInput, (x) => {
  if (searchInput.value == "") {
    ...
    searchList = implementationList.value.filter((list) =>
  ...
  if (confirm(text) == true) {
    try {
      const response = await axios.delete(
        import.meta.env.VITE_API_URL + `implementation_by/${Implementation_id}`,
        {
          headers: {
...
const fetchImplementation = async () => {
  const response = await axios
    .get(import.meta.env.VITE_API_URL + "implementation_by")
    .then((data) => {
...
const submitImplementation = async () => {
  ...
  try {
    const response = await axios.post(
      import.meta.env.VITE_API_URL + "implementation_by",
      testImplementation,
      {
        headers: {
          "user-token": JSON.parse(localStorage.getItem("new_token"))
