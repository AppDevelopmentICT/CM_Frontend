<template>
    <div class="flex flex-row justify-between px-8">
        <h1>Customer</h1>
        <button v-on:click="addCustomer" class="bg-[#908c13] p-2 text-white flex items-center rounded-md">
            ...
            <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-hidden">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <tr v-for="customer in currList" :key="customer.customer_id"
                                ...
                                <td>
                                    ...
                                    :class="!customer.employee || !customer.customer_fullname || !customer.customer_field ? 'bg-yellow-300' : ''"></span>
                                </td>
                                <td>{{ customer.customer_name }}</td>
                                <td>{{ customer.customer_fullname }}</td>
                                <td>{{ customer.customer_field }}</td>
                                <td>{{ customer.employee }}</td>
                                <td><button class="bg-red-600 p-2 rounded-md text-white"
                                        onclick="event.stopPropagation()"
                                        v-on:click="deleteCustomer(customer.customer_id)">Delete</button></td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="w-full">
                        <div class="flex justify-end mt-2">
                            <span class="w-4 h-4 bg-yellow-300 rounded-[50%] self-center mr-1 inline-block"></span>
                            <h4 class="text-xs text-gray-400">Data Entry <span class="font-semibold">Incomplete</span></h4>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="fixed inset-0 flex items-center  justify-center bg-gray-800 bg-opacity-75" v-if="showModal">
        <div class="bg-white rounded-lg p-8 min-w-[400px]  max-w-md">
            ...
            <h1 v-if="customerForm.customer_id==''" class=" text-center mt-4 text-[#908c13] font-semibold text-2xl mb-4">Add Customer</h1>
            <h1 v-else class=" text-center mt-4 text-[#908c13] font-semibold text-2xl mb-4">Edit Customer</h1>
            <div class="py-2 ">
                <label class="block">Customer Name<span class="text-red-600">*</span></label>
                <input type="text" class="inputBox"  v-model="customerForm.customer_name">
            </div>
            <div class="py-2">
                <label class="block">Customer Full Name<span class="text-red-600">*</span></label>
                <input type="text" class="inputBox"  v-model="customerForm.customer_fullname">
            </div>
            <div class="py-2">
                <label class="block">Customer Field<span class="text-red-600">*</span></label>
                <input type="text" class="inputBox"  v-model="customerForm.customer_field">
            </div>
            <div class="py-2">
                <label class="block">Customer Employee Count<span class="text-red-600">*</span></label>
                <Dropdown v-model="customerForm.employee" filter :options="employeeList" placeholder="" id="dropdown"
            ...
            <div class="flex justify-end">
                <button class="bg-[#908c13] text-white px-2 py-1 rounded-md "
                    v-on:click="submitCustomer">submit</button>
            </div>
        </div>
...
const employeeList = ['1-50 Employees','51-100 Employees','101-500 Employees','501-1000 Employees','1001-5000 Employees','5000+ Employees']
...
watch(searchInput, (x) => {
    ...
    if(searchInput.value == ''){
        ...
        if(searchType.value == 'Name'){
            searchList = customerList.value.filter((list) => list.customer_name.toLowerCase().includes(searchInput.value.toLowerCase()))
        }
        else{
            searchList = customerList.value.filter((list) => list.customer_fullname.toLowerCase().includes(searchInput.value.toLowerCase()))
        }
    }
...
const editCustomer = async (customer_id) => {
    const response = await axios.get(import.meta.env.VITE_API_URL + `customer/${customer_id}`)
    .then(data => {
    ...
    let text = "Are you sure you want to delete data?";
        if (confirm(text) == true) {
            try {
                ...
                const response = await axios.delete(import.meta.env.VITE_API_URL + `customer/${customer_id}`,{
                    headers: {
...
const validate = () => {
    valid = false
    if(customerForm.value.customer_name == '' ||
        customerForm.value.customer_fullname == ''||
        customerForm.value.customer_field == ''||
        customerForm.value.employee == ''
    ){
...
const submitCustomer = async () => {
    ...
    if(customerForm.value.customer_id == ''){
        try {
            const response = await axios.post(import.meta.env.VITE_API_URL + "customer", customerForm.value,{
                headers: {
                    "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
                }
            })
        ...
        try {
            const response = await axios.patch(import.meta.env.VITE_API_URL + `customer/${customerForm.value.customer_id}`, customerForm.value,{
                headers: {
                    "user-token": JSON.parse(localStorage.getItem('pocketbase_auth')).token
                }
            })
