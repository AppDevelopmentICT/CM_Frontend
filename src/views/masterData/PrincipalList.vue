<template>
            ...
            <tbody>
              ...
              >
                ...
                <button
                  ...
                  v-on:click="deletePrincipal(principal.principal_id)"
                >
                  Delete
                </button>
              </tr>
  ...
  >
    <div class="bg-white rounded-lg p-8 max-w-md">
      ...
      <div class="py-2">
        <label class="block"
          >Brand Name<span class="text-red-600">*</span></label
        >
        <input type="text" class="inputBox" v-model="principalName" />
      </div>

      <div class="flex justify-end">
        <button
          class="bg-[#908c13] text-white px-2 py-1 rounded-md"
          v-on:click="submitPrincipal"
        >
          submit
        </button>
      </div>
...
watch(searchInput, (x) => {
  if (searchInput.value == "") {
    ...
    searchList = principalList.value.filter((list) =>
      list.principal_name
        .toLowerCase()
  ...
  if (confirm(text) == true) {
    try {
      ...
      const response = await axios.delete(
        import.meta.env.VITE_API_URL + `principal/${principal_id}`,
        {
          headers: {
