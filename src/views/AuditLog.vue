<template>
    <div class="card">
        <DataTable :value="tableData" paginator :rows="20" :rowsPerPageOptions="[5, 10, 20, 50]">
...
const getUserAudit = async () => {
    let roles
    const jwt = JSON.parse(localStorage.getItem('pocketbase_auth')).token
    try {
        ...
        if(roles == "Super Admin") {
            ...
            const response = await axios.get(import.meta.env.VITE_API_URL+"audit/user", {
                headers: {
                    "user-token": jwt
                }
            })
