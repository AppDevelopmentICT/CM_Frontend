      ...
      <div class="">
        <div class="flex justify-between items-center mb-2">
          <div class="flex gap-1">
            ...
            <router-link
              v-if="
                userStore.role == 'Sales' || userStore.role == 'Sales Admin'
              "
              :to="{ path: `/contracts/${projectDetail.project_id}/edit` }"
            >
              <i
                class="pi pi-pencil text-[10px] text-[#908c13] bg-black p-1 rounded-md bg-opacity-10"
              ></i>
            </router-link>
          </div>
          <div class="flex gap-2">
            <div class="flex gap-2">
              <button
                class="bg-[#908c13] p-2 w-48 text-white rounded-md text-sm"
              ...
              <DeleteContract :id="route.params.id" />

              <button
                class="bg-green-600 p-2 w-24 text-white rounded-md text-sm"
                @click="approvedContract()"
                v-if="
                  projectDetail.project_status == 'Pending' &&
                  isApprover &&
                  userStore.role == 'Sales'
                "
              >
                Approve
              </button>
          ...
          <div class="subgroup">
            <div class="flex justify-between items-center">
              ...
              <p
                ...
                :class="{
                  'bg-yellow-500': projectDetail.project_status === 'Pending',
                  'bg-gray-400': projectDetail.project_status === 'Draft',
                  'bg-green-500': projectDetail.project_status === 'Approved',
                  'bg-red-600': projectDetail.project_status === 'Rejected',
                }"
              >
              ...
              <div class="">
                <div class="">
                  <label for="" class="block"
                    >Amount Reserve for Internal Cost</label
                  >
            ...
            <div class="">
              ...
              <a
                ...
                :href="
                  ...
                  file.id +
                  '/' +
                  file.contract_file[0] +
                  '?thumb=100x300'
                "
                target="_blank"
                class="flex gap-1 items-center hover:bg-slate-200 rounded-md px-1 my-1"
              >
...
import DeleteContract from "../components/PageComponents/Contracts/DeleteContract.vue";
...
const duplicateContract = async () => {
  ...
  for (var i in projectDetail.value.product_info) {
    dupeProductCM = JSON.parse(
      JSON.stringify(
        projectDetail.value.product_info[i].corrective_maintenance,
      ),
    );
    ...
    dupeProductPM = JSON.parse(
      JSON.stringify(
        projectDetail.value.product_info[i].preventive_maintenance,
      ),
    );
    ...
    dupeProductSLA = JSON.parse(
      JSON.stringify(projectDetail.value.product_info[i].sla),
    );
    ...
    dupeProductImplementation = JSON.parse(
      JSON.stringify(projectDetail.value.product_info[i].implementation),
    );
    const implementationResponse = await axios.post(
      import.meta.env.VITE_API_URL + "maintenance/implementation",
      dupeProductImplementation,
      header,
    );
    dupeProductImplementation.implementation_id =
      implementationResponse.data.id;

    dupeProduct = JSON.parse(
      JSON.stringify(projectDetail.value.product_info[i]),
    );

    const response = await axios.post(
      import.meta.env.VITE_API_URL +
        `product/project/${projectResponse.data.project_id}`,
      dupeProduct,
      {
        headers: {
          "project-id": projectResponse.data.project_id,
  ...
  router.push(
    `/contracts/${projectResponse.data.project_id}/edit?duplicate=True`,
  );
};
...
const checkApprover = async () => {
  const response = await axios
    .get(import.meta.env.VITE_API_URL + "isApprover", {
      headers: {
  ...
  const record = await pb
    ...
    .then((data) => {
      (projectFiles.value = data.items),
        (pb_id.value = data.id),
        (fileRoot =
          import.meta.env.VITE_PB_BASEURL + "/api/files/nh8a6wpfc1smgyp/");
    });
};
...
const approvedContract = async () => {
  ...
  try {
    const response = await axios.patch(
      import.meta.env.VITE_API_URL + `project/approve/${route.params.id}`,
      null,
      {
        headers: {
          "user-id": userStore.db_id,
...
const rejectContract = async () => {
  ...
  try {
    const response = await axios.patch(
      import.meta.env.VITE_API_URL + `project/reject/${route.params.id}`,
      null,
      {
        headers: {
          "user-id": userStore.db_id,
