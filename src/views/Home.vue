          ...
          <router-link to="/contracts">
            ...
            >
              ...
              <div class="flex mb-0 w-full h-full">
                ...
                <p
                  class="text-md mt-3 self-end ml-auto bg-white font-semibold p-1 rounded-sm"
                  :class="{
                    'text-green-600': totalProjectDiff > 0,
                    'text-red-600': totalProjectDiff < 0,
                  }"
                  v-tooltip.left="{
                    value: 'Project Monthly Diff',
                    showDelay: 500,
                    hideDelay: 200,
                  }"
                >
                  <span v-if="totalProjectDiff > 0">
                    <i class="pi pi-wave-pulse"></i> +
                  </span>
                  <span v-else-if="totalProjectDiff == 0">
                    <i class="pi pi-wave-pulse"></i>
                  </span>
            ...
            >
              <div class="flex w-full">
                ...
                <h4 class="text-lg font-semibold">
                  <span v-if="userStore.role == 'Sales'"
                    >Total Project Handle</span
                  >
          ...
          <div class="flex gap-2" v-if="label1 && data1">
            <div class="self-center m-auto flex-1 h-[350px]">
              <BarChart
                :bar_labels="label1"
                :bar_data="data1"
                bar_title="Top 5 Customer by Contract"
              ></BarChart>
            </div>
          </div>
      ...
      <div class="h-full w-full mt-3">
        <section class="flex flex-col justify-between">
            ...
            <div class="rounded-lg shadow-lg bg-slate-800 flex flex-col">
              <section class="flex flex-col p-5">
                <img
                  ...
                  class="w-16 h-16 select-none rounded-full m-auto bg-white p-[2px]"
                  alt="avatar"
                />
                <div class="text-center text-white mt-2">
                  <h4 class="tracking-wider text-lg font-semibold">
                    {{ userStore.name }}
                  </h4>
                  <p class="text-sm text-gray-300">
                    {{ userStore.role
                    }}<span
                      ><i class="pi pi-building ml-1"></i>Infracom
                      Technology</span
                    >
                  </p>
                </div>
          ...
          <div class="rounded-lg mt-3" v-if="tempData">
            <h4 class="py-2 text-lg text-[#908c13] font-semibold">
              Latest Contracts
            </h4>
...
const getpendingProject = async () => {
  let response = await axios.get(
    import.meta.env.VITE_API_URL + "dashboard/pending-project",
    {
      headers: {
...
const getTotalCustomer = async () => {
  let response = await axios.get(
    import.meta.env.VITE_API_URL + "dashboard/customer",
  );
...
const getTotalProject = async () => {
  let response = await axios.get(
    import.meta.env.VITE_API_URL + "dashboard/project",
    {
      headers: {
        user: `${userStore.db_id}`,
      },
...
const getTopCustomer = async () => {
  let response = await axios.get(
    import.meta.env.VITE_API_URL + "dashboard/top-customers",
  );
...
const getTotalCreatedContract = async () => {
  try {
    const response = await axios.get(
      import.meta.env.VITE_API_URL + "dashboard/total-created",
      {
        headers: {
          user: `${userStore.db_id}`,
        },
...
const getProjectByUser = async () => {
  try {
    const response = await axios.get(
      import.meta.env.VITE_API_URL + `dashboard/created-by/${userStore.db_id}`,
    );
    if (response.data.data.length > 0) {
      tempData.value = response.data.data.map((contract) =>
