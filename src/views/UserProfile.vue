<template>
  ...
  <div class="flex gap-5">
      ...
      <div class="py-2">
        <div class="flex flex-col gap-3">
          <div class="border border-black p-2">
            ...
            <input
              type="text"
              class="w-full my-2 border-b-2 border-black focus:outline-none disabled"
              v-model="userEditData.username"
              v-bind:disabled="editable == false"
            />
          </div>
          <!--
                    <div class="border border-black p-2">
          ...
          <div class="border border-black p-2">
            ...
            <p class="text-[14px] text-gray-500 font-[14px]">
              {{ userEditData.user_roles }}
            </p>
          </div>
          <div class="border border-black p-2 flex gap-8">
            <div class="">
              ...
              <input
                type="checkbox"
                v-model="userEditData.isapprover"
                v-bind:disabled="editable == false"
              />
            </div>
            <div class="">
          ...
          <div class="flex gap-4 justify-end">
            <button
              v-if="userData.user_roles == 'Super Admin'"
            ...
            <button
              class="bg-[#908c13] p-2 text-white rounded-md"
            ...
            <button
              class="bg-orange-500 p-2 text-white rounded-md"
              @click="cancelClick"
              :class="{ hidden: !editable }"
            >
              Cancel Changes
            ...
            <button
              class="bg-emerald-600 p-2 text-white rounded-md"
              @click="saveClick"
              :class="{ hidden: !editable }"
            >
              Save Changes
    ...
    <div
      v-if="userData.user_roles == 'Sales'"
      class="w-full h-fit flex flex-col gap-7"
    >
      <section class="flex gap-5 justify-center">
        <div
          class="h-36 w-72 bg-green-500 flex flex-col p-3 shadow-sm shadow-gray-400 rounded-md select-none text-white"
        >
        ...
        <div class="">
          <div
            class="h-36 w-72 flex flex-col z-50 p-3 shadow-sm shadow-gray-400 rounded-md select-none text-black"
          >
            <div class="flex w-full">
              <span
                class="w-3 h-3 self-center mr-1"
                :class="{
                  'bg-green-600': userData.isapprover === true,
                  'bg-red-600': userData.isapprover === false,
                }"
              ></span>
              <h4 class="text-lg font-semibold">Contract Approved</h4>
              <i
                class="pi self-center ml-auto"
                :class="{
                  'pi-check': userData.isapprover === true,
                  'pi-times': userData.isapprover === false,
                }"
              ></i>
            </div>
            <div class="flex mb-0 w-full h-full">
              <p
                v-if="userData.isapprover"
                class="text-4xl mt-3 self-center font-bold"
              >
                {{ userApprovedProject }}
              </p>
              <p
                v-else
                class="text-lg mt-3 self-center text-red-600 w-full text-center font-bold"
              >
                Can't approve project
              </p>
            </div>
          </div>
        </div>
      ...
      <section class="px-6">
        <div class="flex flex-col">
          <div class="flex justify-between">
            ...
            <button
              class="bg-[#908c13] my-2 p-1 text-white rounded-md text-sm"
              @click="tableCondition = !tableCondition"
            >
              Change Table
            </button>
          </div>
  ...
  <div
    class="fixed inset-0 flex items-center z-50 justify-center bg-gray-800 bg-opacity-75"
    v-if="showChangePass"
  >
...
const getUserbyId = async () => {
  try {
    ...
    const response = await axios.get(
      import.meta.env.VITE_API_URL + `user/${route.params.id}`,
    );
...
const saveClick = async () => {
  ...
  try {
    const record = await pb
      .collection("users")
      .update(userEditData.value.user_id, pbbody);
    try {
...
const getUserProjectHandle = async () => {
  try {
    const response = await axios.get(
      import.meta.env.VITE_API_URL + `user-dashboard/project`,
      {
        headers: {
...
const getTotalApprovedProject = async () => {
  try {
    const response = await axios.get(
      import.meta.env.VITE_API_URL + `user-dashboard/approved`,
      {
        headers: {
          user: `${userData.value.user_id}`,
        },
