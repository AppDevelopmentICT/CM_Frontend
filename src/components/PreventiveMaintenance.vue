<template>
    <div>
        ...
        <div class="pb-2">
            <label for="" class="block">Maintenance By<span class="text-red-600">*</span></label>
            <Dropdown v-model="pmObj" :options="PMList.data" optionLabel="name" placeholder="" id="dropdown"
        ...
        <div v-show="pm.pm_by != 'None' && pm.pm_by != ''">
            ...
            <div class="flex gap-4">
                <div class="w-full">
                    <label for="" class="block">Maintenance Period<span class="text-red-600">*</span></label>
                    <Dropdown v-model="periodeObj" :options="periodeList" optionLabel="periode" placeholder="" id="dropdown"
                    pt:input:class="p-0 text-sm" pt:clearIcon:class="text-black" pt:trigger:class="text-black"
                    class="inputBox"/>   
                </div>
                <div class="w-full">
...
const getPeriode = async () => {
    ...
    .then(data => {
            ...
            periodeObj.value = periodeList.value.find(({periode}) => periode === pm.value.pm_periode)
        })
        .catch(error => {
...
onMounted(async() => {
    ...
    const response = await axios.get(import.meta.env.VITE_API_URL + `pm_by`)
        .then(data => {
            ...
            pmObj.value = PMList.value.data.find(({name}) => name === pm.value.pm_by)
        })
        .catch(error => {
            console.error('Error fetching data:', error);
        });
