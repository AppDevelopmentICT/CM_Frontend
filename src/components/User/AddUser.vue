<template>
    ...
    <div class="w-full">
      ...
      <div class="p-2 w-full">
        ...
        <div class="flex">
          ...
          />
          <span
            class="bg-gray-400 min-w-fit text-md text-white rounded-sm p-2 my-2"
            >@infracom-tech.com</span
          >
        </div>
      </div>
      <div class="p-2 w-full">
        <label for="roles" class="block"
          >Roles<span class="text-red-600">*</span></label
        >
        <Dropdown
          v-model="data.user_roles"
          :options="roleList"
          placeholder=""
          id="dropdown"
      ...
      <div class="flex justify-evenly">
        ...
        <div class="p-2 flex items-center gap-2">
          <input
            id="loginEnabled"
            type="checkbox"
            :disabled="data.isApprover == true"
            v-model="data.createAccount"
            class="w-4 h-4 mt-1"
          />
    ...
    <div class="w-full">
      ...
      <div class="flex justify-between w-full px-2">
        <p class="text-sm self-center">
          Default Password:
          <span class="underline font-bold">ICT_{{ passwordPlaceholder }}</span>
        </p>
        <button class="p-1 bg-orange-300 text-white text-sm rounded-sm hidden">
          Set Password
        </button>
      </div>
    </div>
  ...
  <div class="w-full flex">
    <button
      @click="submitForm()"
      class="bg-emerald-600 text-white p-[6px] rounded-md ml-auto w-[120px] flex items-center justify-center"
    >
...
const getSecretKey = async () => {
  try {
    const response = await axios.get(
      import.meta.env.VITE_API_URL + "secret-key",
      {
        headers: {
...
const submitForm = () => {
  if (data.value.password == "") {
    ...
    data.value.confirm_password =
      "ICT_" + data.value.username.replace(/\s+/g, "");
    insertData();
  } else {
