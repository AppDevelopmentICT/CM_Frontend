          ...
          <table class="min-w-full">
            <thead class="bg-[#908c13] border-b">
              <tr>
                ...
                <th class="text-sm font-medium text-white px-6 py-4">Login</th>
                <th class="text-sm font-medium text-white px-6 py-4 text-left">
                  Details
                </th>
              </tr>
              ...
              >
                ...
                <td class="w-fit">
                  <input
                    ...
                    id=""
                    disabled
                    v-bind:checked="user.isapprover"
                  />
                </td>
                <td class="w-fit">
                  <input
                    ...
                    id=""
                    disabled
                    v-bind:checked="user.isLogin"
                  />
                </td>
                <router-link :to="{ path: `user/${user.id}` }">
                  <button
                    class="bg-emerald-800 p-2 rounded-md text-white m-2 px-2"
                  >
                    Users Details
                  </button>
                </router-link>
...
const fetchUser = async () => {
  try {
    ...
    const response = await axios.get(
      import.meta.env.VITE_API_URL + "all_user",
      {
        headers: {
          "user-id": `${store.db_id}`,
        },
