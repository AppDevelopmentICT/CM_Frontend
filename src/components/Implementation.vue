<template>
  <div>
    ...
    <div
      class="pb-2"
      v-if="
        implementation.implementation_type == undefined ||
        implementation.implementation_type == 'None' ||
    ...
    >
      ...
      <Dropdown
        v-model="impleObj"
        :options="ImpleList.data"
        optionLabel="implementation_type"
        placeholder=""
        id="dropdown"
    ...
    <div v-else class="flex flex-col">
      ...
      <div class="w-full pb-2">
        ...
        >
        <input type="date" class="inputBox" v-model="implementation.end_date" />
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  const response = await axios
    ...
    .then((data) => {
      ...
      impleObj.value = ImpleList.value.data.find(
        ({ implementation_type }) =>
          implementation_type === implementation.value.implementation_type,
      );
    })
    .catch((error) => {
...
watch(impleObj, async () => {
  if (impleObj.value != undefined) {
    implementation.value.implementation_type =
      impleObj.value.implementation_type;
  }
});
</script>
