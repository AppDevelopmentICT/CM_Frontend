...
const handleMouseOut = (event: any) => {
  if (
    !event.relatedTarget ||
    (!event.relatedTarget.closest("#master-data-dd") &&
      !event.relatedTarget.closest("#master-data-list"))
  ) {
  ...
  >
    ...
    <section class="flex flex-col mt-3 h-full">
      <div class="flex flex-col cursor-pointer">
        ...
        <p class="text-neutral-400 text-xs">{{ store.role }}</p>
      </div>
      ...
      <section class="flex flex-col mt-6 gap-2 relative">
        <RouterLink
          ...
          v-if="store.role != 'Helpdesk'"
        >
          <div
            class="hover:bg-[#908c13] hover:text-white rounded-lg flex p-3 text-neutral-400 gap-4 transition"
          >
        ...
        <section
          ...
          v-if="store.role == 'Sales Admin'"
        >
          <div
            class="hover:bg-[#908c13] hover:text-white rounded-lg flex p-3 text-neutral-400 gap-4 transition cursor-pointer"
            id="master-data-dd"
          >
          ...
          <section
            id="master-data-list"
            class="w-full pl-8 flex flex-col gap-1 mt-1"
            v-if="showDropdown"
            @mouseover="handleMouseOver"
            @mouseout="handleMouseOut"
          >
            <RouterLink to="/brand">
              <div
                class="hover:bg-[#908c13] hover:text-white rounded-lg flex p-1 px-2 text-neutral-400 transition text-[13px]"
              >
                <p class="">Brand</p>
              </div>
            </RouterLink>
            ...
            <RouterLink to="/implementation_by">
              <div
                class="hover:bg-[#908c13] hover:text-white rounded-lg flex p-1 px-2 text-neutral-400 transition text-[13px]"
              >
